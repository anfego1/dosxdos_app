...
if (extension_loaded('soap')) {
    ...
    if ($mensaje) {
        /* temp print */
        //print $mensaje;
        $response = $answer->ok('Sincronización realizada: ' . $mensaje);
    } else {
        $response = $answer->ok('Sincronización realizada con éxito');
    }
    ...
    $query = "UPDATE sinc SET fecha=\"$fecha\", estado=\"$estado\", ejecucion=\"$ejecucion\", detalleError=\"$detalleError\" WHERE id = 1";
    $result = $conexion->datos($query);
    $response = $answer->error_500('500 - No existe o no está configurada la extensión Soap en la solicitud de OTS en el servidor donde está alojada la api intermedia');
    $response = json_encode($response);
    //echo $response;
}
