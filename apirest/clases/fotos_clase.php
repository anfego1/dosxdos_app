...
class Fotos extends conexion
{
    ...
    public function post($json)
    {
        try {
            ...
            if (!isset($datos['ruta']) || !isset($datos['linea']) || !isset($datos['fotos']) || !isset($datos['ot']) || !isset($datos['lineaActividad']) || !isset($datos['usuario'])) {
                ...
                if (!$result->num_rows) {
                    ...
                    $this->table = $tablaFotos;
                    $this->tablaLineas = $tablaLineas;
                    $this->tablaFirmas = $tablaFirmas;
                ...
                $filePathAno = __DIR__ . "/../../fotos/" . $this->anoOt;
                if (!file_exists($filePathAno)) {
                    mkdir($filePathAno, 0777, true);
                }
                // Ruta donde se almacenarán los archivos
                $filePath = __DIR__ . "/../../fotos/" . $this->anoOt . "/" . $nombreCarpeta;
                // Crea la carpeta si no existe
                if (!file_exists($filePath)) {
                    mkdir($filePath, 0777, true);
                }
                foreach ($this->fotos as $index => $imageData) {
    ...
    public function fotos($ruta, $linea, $ot, $lineaOt)
    {
        try {
            $this->ot = $ot;
            $query = "SELECT * FROM ot WHERE nombre = '$this->ot'";
            ...
            if (!$result->num_rows) {
                $respuestas = new Respuestas;
                $answer = $respuestas->ok($this->fotos);
                $this->respuesta = $answer;
            } else {
                $row = mysqli_fetch_assoc($result);
                $this->table = 'fotos' . $row['ano'];
            ...
            $respuestas = new Respuestas;
            $answer = $respuestas->error_500('500 - Error en la función fotos() de la clase Fotos: ' . $e->getMessage());
            $this->error = $answer;
        }
    }
    ...
    public function delete($json)
    {
        ...
        try {
            ...
            if (!isset($datos['ot']) || !isset($datos['id']) || !isset($datos['usuarioElimina']) || !isset($datos['tipo'])) {
                ...
                if (!$result->num_rows) {
                    $respuestas = new Respuestas;
                    $answer = $respuestas->error_400('Error 400: La foto no puede ser eliminada ya que las fotos de la OT no existen en la base de datos');
                    $this->error = $answer;
                } else {
                    $row = mysqli_fetch_assoc($result);
                    $this->table = 'fotos' . $row['ano'];
                    ...
                    if ($result) {
                        ...
                        if ($result) {
                            ...
                            $result = parent::datos($query);
                            if ($result) {
                                $archivo = __DIR__ . "/../../fotos/" . $row['ano'] . '/' . $row['ot'] . '/' . $row['nombre'];
                                $nuevaRuta = __DIR__ . "/../../reciclaje/" . $row['nombre'];
                                // Mover el archivo
                                if (rename($archivo, $nuevaRuta)) {
                                    $respuestas = new Respuestas;
                                    $answer = $respuestas->ok('Foto eliminada exitosamente');
                                    $this->respuesta = $answer;
                                } else {
                                    $respuestas = new Respuestas;
                                    $answer = $respuestas->error_500('500 - Error al mover la foto a la carpeta de reciclaje');
                                    $this->error = $answer;
                                }
                            } else {
                                $respuestas = new Respuestas;
                                $answer = $respuestas->error_500('500 - Error en la eliminación sql de los datos de la foto en la tabla de fotos');
                                $this->error = $answer;
                            }
                        } else {
                            $respuestas = new Respuestas;
                            $answer = $respuestas->error_500('500 - Error en la inserción sql de los datos de la foto en la tabla de reciclaje');
                            $this->error = $answer;
                        }
                    } else {
                        $respuestas = new Respuestas;
                        $answer = $respuestas->error_500('500 - Error en la consulta sql de los datos de la foto');
                        $this->error = $answer;
                    }
                }
            }
            ...
            $respuestas = new Respuestas;
            $answer = $respuestas->error_500('500 - Error en la función delete() de la clase Fotos: ' . $th->getMessage());
            $this->error = $answer;
        }
    }
    ...
    public function visible($json)
    {
        try {
            ...
            if (!isset($datos['id']) || !isset($datos['tabla']) || !isset($datos['accion']) || !isset($datos['usuario'])) {
                ...
                if ($result) {
                    $respuestas = new Respuestas;
                    $answer = $respuestas->ok('La imagen ahora es visible para el cliente');
                    $this->respuesta = $answer;
                } else {
                    $respuestas = new Respuestas;
                    $answer = $respuestas->error_500('500 - Error en la consulta sql, no se ha podido poner la imagen visible al cliente');
                    $this->error = $answer;
                }
            }
        } catch (\Throwable $th) {
            $respuestas = new Respuestas;
            $answer = $respuestas->error_500('500 - Error en la función visible() de la clase Fotos: ' . $th->getMessage());
            $this->error = $answer;
        }
    }

    public function fotosOt($ot)
    {
        try {
            $this->ot = $ot;
            $query = "SELECT * FROM ot WHERE nombre = '$this->ot'";
...
/*$_fotos = new Fotos;
//$json = file_get_contents('fotos.json');
$_fotos->fotos('R-0000', '20000', 'OT-20528');
echo $_fotos->respuesta;
echo $_fotos->error;*/
